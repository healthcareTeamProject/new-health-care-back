...
CREATE TABLE IF NOT EXISTS `health_care`.`board` (
  ...
  INDEX `customer_board_idx` (`user_id` ASC) VISIBLE,
  CONSTRAINT `customer_board`
    FOREIGN KEY (`user_id`)
...
CREATE TABLE IF NOT EXISTS `health_care`.`health_schedule` (
  ...
  `health_schedule_start` VARCHAR(20) NOT NULL COMMENT '스케줄 등록을 위한 시작날짜\\\\\\\\n',
  `health_schedule_end` VARCHAR(20) NOT NULL COMMENT '스케줄 등록을 위한 마지막날짜\\\\\\\\n',
...
CREATE TABLE IF NOT EXISTS `health_care`.`meal_schedule` (
  ...
  `meal_schedule_start` VARCHAR(20) NOT NULL COMMENT '스케줄 등록을 위한 시작날짜',
  `meal_schedule_end` VARCHAR(20) NOT NULL COMMENT '스케줄 등록을 위한 마지막날짜\\\\\\\\n',
